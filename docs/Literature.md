...
Reported 1.3–6.7x iteration and 3–25.4x equality over HAMT, with a smaller footprint. The canonicality is the load-bearing part: it makes "same content implies same shape" true, and fast equality and fast diff are both built on that.
...
- **Bolívar Puente, ICFP 2017** (§1) — the C++ account. Its `vector` is this structure and its `flex_vector` is §10's, both over one node type, which is why a `vector` converts to a `flex_vector` in constant time and not the reverse.
...
What differs from the map is worth stating, because it decides the layout. A map node is packed to the slots it uses and reached through a bitmap; a vector node is dense and indexed straight, and its depth is a function of the size alone rather than of how the keys fall. That makes the branching factor a plain trade: a wider node is a shallower trie, which is one fewer dependent load per read, and it is more bytes to copy on a write and more children to claim on a cut. Both halves are measurable, and `VECTOR_BITS` exists to measure them.

## 10. The relaxed vector

The third structure implemented here, and §9's with one rule given up: a node need no longer be full. A node that is not carries a table of running subtree sizes, so an index becomes a short search per level instead of a shift and a mask — and concatenation, a cut from either end, and insertion and erasure in the middle all become logarithmic.

- **[RRB-Trees](https://infoscience.epfl.ch/record/169879)** (Bagwell & Rompf 2011) → **[RRB Vector](https://dl.acm.org/doi/10.1145/2858949.2784739)** (Stucki et al., ICFP 2015) → **immer ICFP 2017** (§1) → Scala's [Vector rewrite](https://github.com/scala/scala/pull/8534). The rebalancing rule on concatenation — how much slack a merged spine may keep before its children are redistributed — is the whole difficulty, and it is what Stucki et al. get right and Bagwell & Rompf state loosely. The parameter is `e`, the nodes a joined run may take beyond the fewest that would do; 2 here, as in immer and in Bagwell & Rompf's own measurements.
- **[c-rrb](https://github.com/hypirion/c-rrb)**, L'orange's C implementation, and the [thesis](https://hypirion.com/thesis.pdf) behind it. The clearest statement of the concatenation in code: `create_concat_plan` and `execute_concat_plan`, which is the shape `Rebalance` in `Vector.cpp` takes. The bound that makes it work is that the join of two subtries comes back as a node with at most two children, so a merged run is never longer than twice the branching factor.
- **What differs here, and why.** The size table is a node like any other, cut from the same slab: a node holds exactly `Slots` values, and refcounting it means a write that moves no element hands its copy the same table rather than another 512 bytes of it — worth 22 points on `set move` at a thousand elements. A table is kept only where it is needed, so the shape stays a function of the contents: a node whose children are full for their level goes without one, so a strict `Vector` converts to a `FlexVector` for free and a vector nothing has been joined into indexes as fast as a strict one.
- **A cut inherits its tables rather than working them out**, and this is the whole of the `take` and `drop` margins. Measuring each kept child and re-accumulating ran `take` 12% behind immer at a million elements, and it survived setting the branching factor to 5, so it was not the shape. Nothing before a cut from the right moves, so the copy takes the parent's running totals as they stand and works out only the last entry; a cut from the left takes them shifted down by what was dropped in front. Both then read the same totals to decide whether the copy is strict enough to need no table at all. That is parity on `take` and 10 points ahead on `drop`.
- **The tail is kept, and it is not a finger.** Scala's practical lesson is that fixed fingers at *both* ends beat a single movable tail. That is a real gap: `PushFront` here is a concatenation with a vector of one, as immer's is, and measures 20% ahead of immer at a million elements but two orders of magnitude behind a `PushBack`. A front finger is the next thing to build if the front matters.

## 11. State of the art, as of 2026
