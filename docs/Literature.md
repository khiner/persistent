...
- **Simple hash tries in C** — [Wellons, _An easy-to-implement, arena-friendly hash map_](https://nullprogram.com/blog/2023/09/30/) and [nrk, _Hash based trees and tries_](https://nrk.neocities.org/articles/hash-trees-and-tries). Both argue a plain 4-way hash trie with arena allocation is "vastly simpler than an over-engineered HAMT" at competitive speed, and nrk reports **branching above 4 buys almost nothing while costing a lot of memory** in that (uncompressed) design. That contradicts the wide-fanout orthodoxy hard enough to be worth one benchmark — it is the honest simplicity baseline for a project whose stated goal is simplicity.
...
- **`B = 5` or `B = 6`?** This is the highest-value experiment now unlocked. B=6 gives 64-way branching, so depth drops from log32(n) to log64(n) — about one fewer dependent pointer chase at a million entries, aimed squarely at the Cuckoo Trie diagnosis (§7). It costs larger nodes, so more bytes copied per path-copy. The CHAMP paper's B=5 was tuned for the JVM with boxed values and array headers, and C++ with inline PODs is a different tradeoff. Build `B` as a compile-time parameter, sweep it, hardcode the winner. immer already supports B=6 (`get_bitmap_type<6>`), so it can serve as a control on both settings.
- **Does the 4-way arena hash trie (Wellons/nrk) actually lose?** Now a design candidate rather than a benchmark control, and the honest baseline for a project whose stated goal is simplicity. One benchmark settles it, and it should be run before committing to CHAMP's complexity.
...
- **Node header packing.** With immer's field order no longer binding: `datamap` and `nodemap` at B=5 are two 32-bit words, so they pack into one 64-bit word, leaving refcount and kind to a second. At B=6 they are two full words and this stops being free — another input to the `B` decision.
